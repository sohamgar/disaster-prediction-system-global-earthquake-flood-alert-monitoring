...
def predict():
    ...
    if request.method == 'POST':
        ...
        InputX2 = np.asarray(
            [[lat, long, depth, mapdateTotime(date)]],
            dtype=np.float32
        )
        ...
        with tf.Session() as sess:
            ...
            if float(var[0]) > 6:
                ...
                # Only send SMS if all env vars exist
                if all([TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM, TWILIO_TO]):
                    client = Client(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN)

                    client.messages.create(
                        to=TWILIO_TO,
                        from_=TWILIO_FROM,
                        body="⚠️ Earthquake Alert: Predicted magnitude above threshold"
                    )
                else:
